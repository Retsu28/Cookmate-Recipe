...
export default function AIActivityMonitoring() {
  ...
  const fetchActivity = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      setSelectedSave((current) => nextSaves.find((save) => save.id === current?.id) || null);
    } catch (err: any) {
  ...
  const renderExpandedRow = useCallback((save: AdminCameraSave) => (
    <motion.div
      className="p-6 bg-stone-50 border-b border-stone-200"
    ...
    >
      <div className="grid gap-6 md:grid-cols-[200px_1fr]">
        <div className="aspect-square overflow-hidden rounded-[1.5rem] border border-orange-100 bg-gradient-to-b from-orange-50 to-orange-100/50 p-6">
          {save.thumbnailImageData ? (
            <img src={save.thumbnailImageData} alt="" className="h-full w-full object-contain drop-shadow-xl" />
          ) : (
        ...
        <div className="space-y-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-lg font-extrabold text-stone-900">{save.userName}</p>
              <p className="truncate text-sm font-medium text-stone-400">{save.userEmail}</p>
            </div>
            <div className="flex shrink-0 flex-col items-end gap-2">
              <StatusBadge tone={cameraStatusTone(save.status)}>{save.status}</StatusBadge>
              <StatusBadge tone="neutral">{sourceLabel(save.sourceType)}</StatusBadge>
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-stone-100 bg-white p-4">
              ...
              <p className="mt-2 font-extrabold text-stone-900">{formatDateTime(save.createdAt)}</p>
            </div>
            <div className="rounded-2xl border border-stone-100 bg-white p-4">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-stone-400">Matches</p>
              <p className="mt-2 flex items-center justify-between font-extrabold text-stone-900">
                {save.recipeMatchCount}
                {save.confidence && (
                  <StatusBadge tone={confidenceTone(save.confidence)}>
                    {save.confidence}
                  </StatusBadge>
                )}
              </p>
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-stone-400">Detected Details</p>
            <div className="mt-2 flex flex-wrap gap-1.5">
              {save.detectedIngredients.length > 0 ? (
                save.detectedIngredients.map((ingredient) => (
                  <StatusBadge key={ingredient} tone="neutral">{ingredient}</StatusBadge>
                ))
              ) : (
                <span className="text-sm font-medium text-stone-400">No ingredient saved</span>
            ...
            {save.detectedIngredientDescription && (
          ...
          <div className="rounded-2xl border border-orange-100 bg-orange-50 p-4">
            ...
            <p className="mt-2 font-extrabold text-stone-900">
              {save.suggestedRecipe || 'No recipe match saved'}
            </p>
          </div>
        </div>
      </div>
        ...
        >
          {loading ? (
            ...
            <AdminTable
              ...
              expandedRowId={selectedSave?.id || null}
              renderExpandedRow={(save) => selectedSave?.id === save.id ? renderExpandedRow(save) : null}
            />
          )}
        </AdminSectionCard>
      </div>
