...
function friendlyFirebaseError(err: unknown): Error {
  ...
  // Backend API errors (e.g. 429 lockout) — pass through with .data preserved
...
export const authService = {
  ...
  async login(email: string, password: string): Promise<AuthResult> {
    ...
    const data = await api.post<AuthResult | MfaRequiredResult>('/api/auth/login', { email: email.trim(), password });
