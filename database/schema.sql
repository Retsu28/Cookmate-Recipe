...
-- Email-level login rate limiting (tracks failed attempts by email address)
...
CREATE INDEX idx_email_rate_limits_locked_until ON email_rate_limits (locked_until);

-- IP-level login rate limiting (tracks failed attempts by IP address)
...
CREATE INDEX idx_ip_rate_limits_locked_until ON ip_rate_limits (locked_until);
...
CREATE TABLE recipes (
    ...
    image_url TEXT,
    video_filename TEXT,
    instruction_timestamps JSONB DEFAULT '[]'::JSONB,
    video_credits TEXT,
...
CREATE INDEX idx_recipes_category_trgm ON recipes USING GIN (category gin_trgm_ops);
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Recipe views & cooking sessions
-- ═══════════════════════════════════════════════════════════════════════════════

-- Legacy recipe views (superseded by recipe_viewed)
...
-- Tracks which user completed cooking which recipe
CREATE TABLE recipe_cooking_sessions (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    recipe_id INTEGER NOT NULL REFERENCES recipes(id) ON DELETE CASCADE,
...
CREATE INDEX idx_cooking_sessions_user ON recipe_cooking_sessions (user_id);
CREATE INDEX idx_cooking_sessions_recipe ON recipe_cooking_sessions (recipe_id);
...
-- AI Camera rate limiting (3 uses per 24 hours per user)
CREATE TABLE ai_camera_rate_limits (
...
CREATE INDEX idx_ai_camera_rate_limits_window ON ai_camera_rate_limits (user_id, window_start_at);
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Planner notifications & device schedules
-- ═══════════════════════════════════════════════════════════════════════════════
...
CREATE INDEX idx_reviews_recipe_id_hidden ON reviews (recipe_id, is_hidden) WHERE is_hidden = FALSE;

-- Helpfulness votes (3-level: 0 = not helpful, 1 = helpful, 2 = very helpful)
CREATE TABLE review_helpfulness (
    id SERIAL PRIMARY KEY,
    review_id INTEGER REFERENCES reviews(id) ON DELETE CASCADE,
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
    is_helpful BOOLEAN NOT NULL,
    helpfulness_level SMALLINT NOT NULL DEFAULT 1 CHECK (helpfulness_level IN (0, 1, 2)),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Chat conversations (CookMate AI)
-- ═══════════════════════════════════════════════════════════════════════════════
...
CREATE UNIQUE INDEX idx_chat_conversations_user_unique ON chat_conversations (user_id);
