...
export const authService = {
  ...
  async login(email, password) {
    ...
    try {
      ...
      // Call backend directly so failed attempts are tracked in the DB for lockout
      const { data, status } = await api.post('/api/auth/login', { email: email.trim(), password });
      ...
      // Re-throw MfaRequiredError as-is
      if (error instanceof MfaRequiredError) throw error;

      const out = new Error(toMessage(error, 'Unable to sign in. Please try again.'));

      // Preserve backend rateLimit data so the lockout UI hook can read it
      const backendData = error?.response?.data;

      if (backendData?.rateLimit) {

        out.data = backendData;
