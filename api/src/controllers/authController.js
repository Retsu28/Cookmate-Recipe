...
} = require('../services/emailRateLimitService');
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    // Check IP-level lockout first (catches multi-email attackers)
    const ipLockout = await checkIpLockout(clientIp);

    if (ipLockout?.locked) {

      return res.status(429).json({
        error: `Too many failed login attempts from your network. Please try again in ${ipLockout.minutesLeft} minute${ipLockout.minutesLeft !== 1 ? 's' : ''}.`,
        code: 'auth/too-many-attempts',
        rateLimit: {
          locked: true,
          minutesLeft: ipLockout.minutesLeft,
          secondsLeft: ipLockout.secondsLeft,
          lockoutMinutes: ipLockout.lockoutMinutes,
          retryAfter: ipLockout.secondsLeft,
          attemptsRemaining: 0,
        },
      });
    ...
    // Check email-level lockout (tracks by email, even for non-existent accounts)
    const emailLockout = await checkEmailLockout(normalizedEmail);

    if (emailLockout?.locked) {

      const resp = formatRateLimitResponse(
        emailLockout,
        `Too many failed login attempts. Please try again in ${emailLockout.minutesLeft} minute${emailLockout.minutesLeft !== 1 ? 's' : ''}.`
      );

      return res.status(429).json({ ...resp, code: 'auth/too-many-attempts' });
    ...
    if (result.rowCount === 0) {
      ...
      // Record in both email and IP rate limits even for non-existent accounts
      await Promise.all([
    ...
    // Check per-account lockout (users.locked_until)

    if (row.locked_until && new Date(row.locked_until) > new Date()) {

      const minutesLeft = Math.ceil((new Date(row.locked_until) - new Date()) / 60000);

      const secondsLeft = Math.ceil((new Date(row.locked_until) - new Date()) / 1000);
      ...
      return res.status(429).json({
        ...
        rateLimit: {
          ...
          lockoutMinutes: LOGIN_LOCK_MINUTES,
    ...
    // Firebase-only accounts have no password hash — can't log in with email+password
    ...
    if (!ok) {
      ...
      // Record in email_rate_limits, ip_rate_limits, and users.failed_login_attempts
      const [emailRateState, ipRateState] = await Promise.all([
        recordFailedAttempt(normalizedEmail),
        recordIpFailedAttempt(clientIp),
      ]);
      ...
      const attemptsRemaining = Math.max(0, LOGIN_MAX_ATTEMPTS - newAttempts);
      ...
      if (shouldLock || emailRateState?.locked || ipRateState?.locked) {

        const minutesLeft = shouldLock ? LOGIN_LOCK_MINUTES : (emailRateState?.minutesLeft ?? ipRateState?.minutesLeft ?? LOGIN_LOCK_MINUTES);

        const secondsLeft = shouldLock ? LOGIN_LOCK_MINUTES * 60 : (emailRateState?.secondsLeft ?? ipRateState?.secondsLeft ?? LOGIN_LOCK_MINUTES * 60);
    ...
    // Successful login — clear both lockout systems
    await clearEmailRateLimit(normalizedEmail);
...
exports.firebase = async (req, res) => {
  ...
  try {
    ...
    // Check email-level lockout before processing
    const emailLockout = await checkEmailLockout(normalizedEmail);
    ...
    // Successful Firebase login — clear email-level lockout
    await clearEmailRateLimit(normalizedEmail);
